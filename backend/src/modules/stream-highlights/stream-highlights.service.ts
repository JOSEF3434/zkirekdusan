...
export class StreamHighlightsService {
  ...
  async createHighlight(userId: string, streamId: string, data: { title: string; description?: string; startTimeSec: number; endTimeSec: number }) {
    const stream = await this.prisma.liveStream.findUnique({
      where: { id: streamId },
      include: { videoChannel: true },
    });

    if (!stream || stream.deletedAt) throw new NotFoundException('Stream not found');

    // Simple permission check (assuming only creator can highlight for now)
    if (stream.createdById !== userId) {
      throw new ForbiddenException('Only the stream creator can create highlights');
    }
  ...
  async deleteHighlight(userId: string, highlightId: string) {
    const highlight = await this.prisma.streamHighlight.findUnique({ where: { id: highlightId } });
