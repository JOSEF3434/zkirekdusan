// lib/features/home/presentation/providers/video_feed_provider.dart
import 'dart:async';
...
final videoFeedProvider = AsyncNotifierProvider<VideoFeedNotifier, VideoFeedState>(() {
  return VideoFeedNotifier();
});

class VideoFeedNotifier extends AsyncNotifier<VideoFeedState> {
  CancelToken? _cancelToken;
  VideoFeedCategory _currentCategory = VideoFeedCategory.all;
  ...
  Future<VideoFeedState> _fetchInitial(VideoFeedCategory category) async {
    ...
    if (category == VideoFeedCategory.all || category == VideoFeedCategory.latest) {
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      VideoListResponseDto response;
      if (_currentCategory == VideoFeedCategory.all || _currentCategory == VideoFeedCategory.latest) {
        response = await repo.searchVideos(
          page: nextPage, 
          limit: 10, 
          cancelToken: _cancelToken
